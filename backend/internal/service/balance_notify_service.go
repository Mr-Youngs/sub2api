...
func (s *BalanceNotifyService) buildBalanceLowEmailBody(userName string, balance, threshold float64, siteName, rechargeURL string) string {
	rechargeBlock := ""
	if rechargeURL != "" {
		rechargeBlock = `<p><a href="{{recharge_url}}" class="button">立即充值 / Top Up Now</a></p>`
	}
	return renderBrandedNotificationEmailBody(
		...
		siteName,
		`
<p><strong>{{recipient_name}}</strong>，您的账户余额已低于提醒阈值。</p>
<p style="display: inline-block; margin: 8px 0 18px; padding: 14px 22px; border-radius: 7px; background: #f7f6f0; color: #292c3b; font-size: 30px; font-weight: 800;">${{current_balance}}</p>
<p class="muted">当前余额低于 <strong>${{threshold}}</strong>，请及时充值以免服务中断。</p>
<p class="muted">Your account balance has fallen below the alert threshold.</p>
`+rechargeBlock,
		map[string]string{
...
<table role="presentation" cellpadding="0" cellspacing="0">
  <tr><td>账号 ID / Account ID</td><td><strong>#{{account_id}}</strong></td></tr>
  <tr><td>账号 / Account</td><td><strong>{{account_name}}</strong></td></tr>
  <tr><td>平台 / Platform</td><td>{{platform}}</td></tr>
  <tr><td>维度 / Dimension</td><td>{{quota_dimension}}</td></tr>
  <tr><td>已使用 / Used</td><td><strong>${{quota_used}}</strong></td></tr>
  <tr><td>限额 / Limit</td><td>{{quota_limit}}</td></tr>
  <tr><td>剩余额度 / Remaining</td><td><strong>${{quota_remaining}}</strong></td></tr>
  <tr><td>提醒阈值 / Alert Threshold</td><td>{{quota_threshold}}</td></tr>
</table>
<p class="muted">Account remaining quota has fallen below the alert threshold.</p>`,
		map[string]string{
			"account_id":      strconv.FormatInt(accountID, 10),
			"account_name":    accountName,
			"platform":        platform,
