...
// ---------- branded shell checks ----------

func TestBuildBalanceLowEmailBody_UsesBrandedShell(t *testing.T) {
	s := &BalanceNotifyService{}
	body := s.buildBalanceLowEmailBody("u", 1.0, 5.0, "Site", "")

	require.Contains(t, body, `class="site"`)
	require.Contains(t, body, "#292c3b")
	require.Contains(t, body, "#f0c845")
	require.NotContains(t, body, `class="logo"`)
	require.NotContains(t, body, "site_logo_url")
	require.NotContains(t, body, "linear-gradient")
}

func TestBuildQuotaAlertEmailBody_UsesBrandedShell(t *testing.T) {
	s := &BalanceNotifyService{}
